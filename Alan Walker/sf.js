// Get the first img element
    const firstImg = document.querySelector('img');

    // Check if an img element is found
    if (firstImg) {
      // Get the next sibling element
      const nextElement = firstImg.nextElementSibling;

      // Check if a next sibling element is found
      if (nextElement) {
        // Set the ID of the next sibling element to "message"
        nextElement.id = 'message';
      }
    }
